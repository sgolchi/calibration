data {
  ...
  int<lower=1> D;
  real x1[N];                         // x-input values where the field observations                                        // are available
  vector[N] y;                        // field data
}
parameters {
  real<lower=0> lambda_inv;           // variance of the discrepancy term GP
  real<lower=0> beta_x;               // correlation parameters for x
  real<lower=0> sigma_sq;             // observation error variance
  real<lower=0> theta;                // unknown computer model input to be estimated
}

model {
  vector[N] mu;
  matrix[N,N] Sigma;                 // covariance matrix of the combined data
  matrix[N,N] CH;                    // cholesky decomposition of the covariance matrix
    for (i in 1:(N-1)) {
    Sigma[i,i] <- lambda_inv + sigma_sq;
    for (j in (i+1):N) {
      Sigma[i,j] <- lambda_inv * exp(- beta_x * pow(x1[i] - x1[j], 2));
      Sigma[j,i] <- Sigma[i,j];
    }
  }
  Sigma[N,N] <- lambda_inv + sigma_sq;
  CH <- cholesky_decompose(Sigma);
  for (i in 1:N) mu[i] <- sqrt(2*x1[i]/theta);
  //priors:
